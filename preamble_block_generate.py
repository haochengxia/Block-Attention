...
def apply_pkv_rerotary_position_embeddings_with_preamble(pkv: DynamicCache, emb: LlamaRotaryEmbedding, len_first_block_kv: int) -> DynamicCache:
    device = pkv.key_cache[0].device
    emb.to(device=device)
    position_ids = torch.arange(start=0, end=pkv.key_cache[0].size(-2), dtype=torch.int64, device=device)
    position_ids = position_ids.unsqueeze(dim=0).repeat(repeats=[pkv.key_cache[0].size(0), 1])
    ...
    # remove the preamble kv from the past key values, which means pkv remove the preamble_kv at the head
    for l_idx in range(0, len(pkv.key_cache)):
        pkv.update(
            pkv.key_cache[l_idx][:, len_first_block_kv:, :],
            pkv.value_cache[l_idx][:, len_first_block_kv:, :],
            l_idx
        )
    print("pkv.key_cache[0].size(-2) after cutting the preamble kv", pkv.key_cache[0].size(-2))
...
def build_block_past_key_values(
    ...
    for b_idx, block in enumerate(blocks):
        ...
        if b_idx == 0:
            ...
            first_block_kv = DynamicCache()
            for layer_idx in range(len(output.past_key_values.key_cache)):
                first_block_kv.update(
                    output.past_key_values.key_cache[layer_idx],
                    output.past_key_values.value_cache[layer_idx],
                    layer_idx
                )
            pkv = apply_pkv_rerotary_position_embeddings(pkv=output.past_key_values, emb=emb)
        else:
            past_key_values = DynamicCache()
            for layer_idx in range(len(first_block_kv.key_cache)):
                past_key_values.update(
                    first_block_kv.key_cache[layer_idx],
                    first_block_kv.value_cache[layer_idx],
                    layer_idx
                )
            output: CausalLMOutputWithPast = model(
                input_ids=block_input_ids, use_cache=True, past_key_values=past_key_values, return_dict=True
            )
            print("output.past_key_values.key_cache[0].size(-2)", output.past_key_values.key_cache[0].size(-2))
            print("first_block_kv.key_cache[0].size(-2)", first_block_kv.key_cache[0].size(-2))
            pkv = apply_pkv_rerotary_position_embeddings_with_preamble(
                pkv=output.past_key_values, emb=emb, 
                len_first_block_kv=first_block_kv.key_cache[0].size(-2)
            )
...
def block_generate(
    ...
    class LogitsMask(LogitsProcessor):
        ...
        def __call__(self, input_ids, scores):
            mask = torch.zeros_like(scores).to(device=model.device)
